...
// 改善点4: 未定義の投稿タイプが来た場合でもPHPエラー（Warning）が出ないように安全に取得
...
<div class="eyecatch -archive">
  ...
  <img src="<?php echo esc_url(get_template_directory_uri() . '/img/page/' . $img_file); ?>" alt="<?php echo esc_attr($archive_title); ?>" width="1920" height="600" fetchpriority="high" decoding="async">
</div>
...
<main class="<?php echo esc_attr($slug . '_page'); ?>">
  ...
  <div class="container">
    <?php if (have_posts()): while (have_posts()) : the_post(); ?>
        ...
        <?php
        // 改善点3: 構造化データ（JSON-LD）をPHP配列で安全に構築し、構文エラーを完全に防止する
        $schema = [];
        if ($is_question) {
          $schema = [
          ...
          // 著者の設定
          $author = $is_introduction
            ? ["@type" => "Organization", "name" => "トータルスマート株式会社", "url" => home_url('/')]
            : ["@type" => "Person", "name" => get_the_author_meta('display_name'), "url" => get_author_posts_url(get_the_author_meta('ID'))];
          ...
          // サムネイルがある場合のみ画像URLをセット（空文字によるSearch Consoleエラーを防止）
              ...
              <div class="disclaimer--inner">
                ...
                <ol class="disclaimer--list">
                  ...
                  <li>
                    ...
                    <p>IT・設備機器の仕様や法規制は頻繁にアップデートされるため、常に最新情報を公式サイトやなどで必ずご確認ください。</p>
                  </li>
                </ol>

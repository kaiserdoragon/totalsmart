...
<head>
  ...
  <?php
  if (is_front_page()): ?>
    <meta name="description" content="<?php echo esc_attr(wp_strip_all_tags(get_bloginfo('description'))); ?>">
  <?php elseif (is_single() || is_page()): ?>
    ...
    if (empty($meta_desc)) {
      $meta_desc = wp_strip_all_tags(get_the_title()) . "について。トータルスマートは愛知・岐阜・三重・静岡でオフィスのコスト削減を支援します。";
    }
    ?>
  ...
  <?php else: ?>
    ...
    $meta_desc = wp_strip_all_tags($page_title) . "について。トータルスマートは愛知・岐阜・三重・静岡でオフィスのコスト削減を支援します。";
    ?>
    <meta name="description" content="<?php echo esc_attr($meta_desc); ?>">
  <?php endif; ?>
      ...
      <div class="header--inner">
        <p class="header--logo">
          ...
          <a href="<?php echo esc_url(home_url('/')); ?>">
            <img src="<?php echo esc_url(get_theme_file_uri('/img/common/logo.png')); ?>"
              alt="トータルスマート株式会社"
              width="325" height="68"
              fetchpriority="high" />
          </a>
